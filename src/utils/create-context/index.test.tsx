...
describe('createContext', () => {
  it('should confirm that createContent returns a context', () => {
    ...
    const [Provider, useContext, displayName] = createContext<
      typeof contextValue
    >('TestContext');
    ...
    expect(
      ((displayName as unknown) as React.Context<object>).displayName
    ).toBe('TestContext');
  });
  ...
  it('should confirm that createContent throws an error when a component not wrapped with the context provider tries to use the context', () => {
    const [, useContext] = createContext<{ name: string | null }>(
      'TestContext'
    );
