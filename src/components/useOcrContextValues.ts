...
export function useOcrContextValues(lang: string, logger?: Logger) {
  ...
  const performOCR = useCallback(async (key: string) => {
    ...
    try {
      const {
        data: { text },
      } = await Tesseract.recognize(selectedImage, ocrLang, { logger });
